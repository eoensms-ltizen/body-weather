...
test("Memory Cut keeps every route in chronological reveal order and lands near 60 seconds", () => {
  ...
  assert.ok(story.highlightRouteIds.length <= 8);
...
test("Premiere metadata uses measured recovery only and labels generated route progress", () => {
  ...
  assert.ok(rides.every((scene) => scene.rideMeta?.paceMode === "distance"));
...
test("Ride scenes draw first, then burn into the revealed atlas", () => {
  const summary = buildVerificationSummary();
  const atlas = buildAtlasModel(summary.activities, false);
  const story = buildPremiereStory(atlas.routes, atlas.achievements, atlas.placeClusters, summary.wellness, DEFAULTS);
  assert.ok(story);
  const ride = story.scenes.find((scene) => scene.kind === "ride");
  assert.ok(ride);

  const drawing = premiereFrameAt(story, ride.startMs + ride.durationMs * 0.5);
  assert.equal(drawing.kind, "ride");
  assert.equal(drawing.revealedCount, ride.revealStart);
  assert.ok(drawing.travelProgress > 0);
  assert.equal(drawing.imprintProgress, 0);

  const earlyBurn = premiereFrameAt(story, ride.startMs + ride.durationMs * 0.84);
  assert.equal(earlyBurn.kind, "ride");
  assert.equal(earlyBurn.revealedCount, ride.revealStart);
  assert.ok(earlyBurn.imprintProgress > 0);

  const sealed = premiereFrameAt(story, ride.startMs + ride.durationMs * 0.94);
  assert.equal(sealed.kind, "ride");
  assert.equal(sealed.revealedCount, ride.revealEnd);
  assert.equal(sealed.travelProgress, 1);
  assert.ok(sealed.imprintProgress > 0.6);
});
