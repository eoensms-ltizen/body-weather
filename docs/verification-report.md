...
배포 URL은 `https://body-weather-ltizen.eoensms168766.chatgpt.site`이며 owner-only 접근으로 게시했습니다. Sites 인증 헤더를 사용한 프로덕션 스모크 요청에서 HTTP 200, `Body Weather`, ZIP 가져오기 마크업을 확인했습니다. 자동화 Chrome 프로필은 허용된 Sites 계정과 달라 권한 화면을 반환했으므로 배포본의 로그인 뒤 상호작용은 실행하지 않았고, 동일 빌드의 상호작용 검증은 로컬 실제 Chrome에서 수행했습니다.
