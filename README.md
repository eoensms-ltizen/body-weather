...
- **Poster Studio**: Atlas에서 적용한 기간·종목·환경 필터를 기본 범위로 이어받아 16:9 4K, 4:5, 9:16 PNG를 Canvas에서 생성합니다. 선택 영역이나 전체 기록으로도 전환할 수 있고, 민감 위치 마스킹이 항상 기본 적용되며 GPS/EXIF 메타데이터를 넣지 않습니다.
- **Data & Privacy**: 실제 보유 필드에 따른 Capability, 제외 미디어, 복구 가능한 오류와 네트워크 경계를 보여줍니다.
- **Local Atlas Reuse**: 사용자가 선택하면 분석 결과 한 세트를 이 브라우저의 IndexedDB에 저장해 새로고침 뒤 이어봅니다. 원본 ZIP과 미디어는 저장하지 않으며 앱 안에서 즉시 삭제할 수 있습니다.
...
기본 지도 스타일은 [CARTO Dark Matter](https://carto.com/basemaps/)를 사용하고 OpenFreeMap을 보조 공급자로 두며, 지도 데이터 저작권 표시는 앱 안에 유지합니다. 지도 타일 요청에는 현재 보고 있는 영역과 네트워크 정보가 공급자에게 전달될 수 있지만, 원본 ZIP·건강값·GPS 경로 좌표는 요청 본문이나 쿼리로 보내지 않습니다. 한 공급자가 실패하거나 12초 안에 준비되지 않으면 다음 공급자로 자동 전환하고, 모든 공급자가 지연돼도 로컬 어두운 배경 위에 경로는 계속 표시합니다.
...
- 기기 저장을 선택하지 않으면 새로고침 때 가져온 데이터가 사라집니다. 선택하면 정규화된 활동·건강·원본 GPS 결과 한 세트가 현재 브라우저 프로필에 암호화 없이 저장됩니다.
- `새 ZIP`은 현재 화면만 초기화하고 기기 저장 데이터는 유지합니다. 첫 화면 또는 Data & Privacy에서 별도로 삭제할 수 있습니다.
